...
- `package.json`（含 `commands` 清单、`build:view` / `build:scripts` 脚本、`@launcher/api`〔+ `@launcher/api-node`〕依赖）
...
- 宿主能力**直连 SDK**：view 侧 `import { exec, host, hostUi, screenshot, storage } from '@launcher/api'`；
  script 侧 `import { ctx, done, fail, log, onError } from '@launcher/api-node'`。
...
- 需要读本地文件时，照抄 `plugins/totp/src/no-view/`：`find-image.ts` 放纯逻辑（可测），`read-image.ts` 是 `ctx/log/done` 胶水。
