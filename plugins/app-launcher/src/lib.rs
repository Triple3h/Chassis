...
// ── 图标（v1 `core/icons.ts`：.icns → sips → PNG → data URL）──────

/// 用系统 `sips` 把 `.icns` 转成 64px PNG 并缓存；结果按 `sha1(icns 路径)` 命名。
...
impl IconCache {
    ...
    pub async fn data_url(&self, icns_path: Option<&str>) -> Option<String> {
        let Some(icns_path) = icns_path else { return None };
        if let Some(cached) = self.memory.lock().unwrap_or_else(|err| err.into_inner()).get(icns_path) {
            return cached.clone();
        }

        let key = format!("{:x}", Sha1::digest(icns_path.as_bytes()));
        let target = self.dir.join(format!("{key}.png"));
        let mut ready = target.exists();
        if !ready {
            let _ = std::fs::create_dir_all(&self.dir);
            ready = convert_with_sips(icns_path, &target).await;
        }
        let url = if ready {
            std::fs::read(&target)
                .ok()
        ...
        self.memory
            ...
            .insert(icns_path.to_string(), url.clone());
        url
    }
}

/// 不接受调用方传入的目标路径：source 来自扫描结果，target 由 hash 生成。
async fn convert_with_sips(source: &str, target: &Path) -> bool {
...
mod tests {
    ...
    fn icon_cache_returns_none_without_source_and_handles_bad_path() {
        ...
        assert!(runtime().block_on(cache.data_url(Some("/no/such/icon.icns"))).is_none(), "sips 失败要安静回落");
