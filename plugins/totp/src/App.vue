...
<template>
  <AppShell>
    <header class="flex items-center gap-2 border-b border-line px-3 py-2">
      <UiIcon name="shield" :size="16" class="text-accent" />
      <span class="text-[13px] font-semibold">双重验证器</span>
      <span class="launcher-chip">{{ accounts.length }}</span>
      <span v-if="vault" class="launcher-chip">
        <UiIcon name="lock" :size="11" />已加密
      </span>
      ...
      <button class="launcher-btn" title="添加（⌘N）" @click="openAdd">
    ...
    <div ref="listEl" class="launcher-scroll relative min-h-0 flex-1">
      <div class="relative" :style="{ height: totalHeight + 'px' }">
        <div
          v-for="(account, i) in visibleRows"
          :key="account.id"
          class="launcher-row absolute inset-x-0 flex h-14 cursor-default items-center gap-3 px-3"
        ...
        >
          ...
          <div class="flex items-center gap-3">
            <span
              class="launcher-code launcher-code-big tabular-nums"
            ...
            <span class="flex w-[46px] items-center justify-end gap-1">
              <template v-if="account.type === 'totp'">
                <svg v-if="settings.showRing" class="launcher-ring" width="22" height="22" viewBox="0 0 22 22">
                  <circle cx="11" cy="11" r="9" stroke="var(--launcher-line)" stroke-width="2.4" />
                  <circle
                    cx="11"
                    cy="11"
                    r="9"
                    stroke="var(--launcher-accent)"
                    stroke-width="2.4"
                    :stroke-dasharray="CIRC"
                    :stroke-dashoffset="CIRC * (1 - progressOf(account))"
                  />
                </svg>
                <span class="launcher-code text-right text-[11.5px] text-faint">{{ remainingSeconds(account) }}</span>
              </template>
            </span>

            <button class="launcher-btn ghost" title="复制" @click.stop="copyAccount(account)">
      ...
      <div v-if="!loading && !filtered.length" class="grid h-full place-items-center px-6 text-center">
        <div class="space-y-2">
          <UiIcon name="shield" :size="26" class="mx-auto text-faint" />
          ...
          <button class="launcher-btn primary mx-auto" @click="dialog = 'import'">
            <UiIcon name="qr" :size="13" />开始导入
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="!unlocked" class="absolute inset-0 z-30 grid place-items-center bg-bg">
      <div class="w-[320px] space-y-3 rounded-xl border border-line bg-panel p-5">
        <div class="flex items-center gap-2">
          <UiIcon name="lock" :size="16" class="text-accent" />
        ...
        <button class="launcher-btn primary w-full justify-center" @click="tryUnlock">
