...
export function highlightJsonLine(line: string): string {
  ...
  while (i < n) {
    ...
    if (c === 34) {
      ...
      out += span(line.charCodeAt(k) === 58 ? 'launcher-hl-key' : 'launcher-hl-str', lit)
      i = j + 1
      continue
    }
    // 数字
    if ((c >= 48 && c <= 57) || (c === 45 && i + 1 < n && line.charCodeAt(i + 1) >= 48 && line.charCodeAt(i + 1) <= 57)) {
      ...
      out += span('launcher-hl-num', line.slice(i, j))
      i = j
      continue
    }
    // 关键字
    if (line.startsWith('true', i) || line.startsWith('null', i)) {
      out += span('launcher-hl-kw', line.slice(i, i + 4))
      i += 4
      continue
    }
    if (line.startsWith('false', i)) {
      out += span('launcher-hl-kw', line.slice(i, i + 5))
      i += 5
      continue
    }
    ...
    out += span('launcher-hl-punc', line.slice(i, j))
    i = j
  }
  return out
}
