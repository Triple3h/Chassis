...
<template>
  ...
  >
    ...
    <div v-if="phase === 'manual'" class="flex flex-col gap-3">
      ...
      <div class="sof-mono sof-scroll max-h-24 rounded-lg border border-line bg-bg p-2.5 text-[11.5px] break-all">
        {{ result?.command }}
      </div>

      <div class="text-[11.5px] text-muted">
        待生效文件：<span class="sof-mono">{{ result?.pendingPath }}</span>
      </div>
    ...
    <div v-else class="flex flex-col gap-3">
      <div class="flex flex-wrap items-center gap-2 text-[12px]">
        <span v-if="diff" class="sof-chip text-addfg">+{{ diff.added }}</span>
        <span v-if="diff" class="sof-chip text-delfg">−{{ diff.removed }}</span>
        <span class="text-muted">{{ rows.length }} 行</span>
        <div class="flex-1" />
        <span v-if="!writable" class="sof-chip">
          <SofIcon name="lock" :size="11" />
          需要管理员授权
        </span>
      </div>

      <div ref="scroller" class="sof-scroll h-72 rounded-lg border border-line bg-bg">
        <div class="relative" :style="{ height: `${totalHeight}px` }">
          <div class="absolute inset-x-0" :style="{ transform: `translateY(${offsetY}px)` }">
            <div
              v-for="row in window$"
              :key="`${row.no}-${row.kind}`"
              class="sof-mono flex h-5 items-center gap-2 px-2 text-[11.5px] leading-5"
              :class="{ 'sof-diff-add': row.kind === 'add', 'sof-diff-del': row.kind === 'del' }"
            >
      ...
      <div v-if="phase === 'writing'" class="flex items-center gap-2 rounded-lg border border-line bg-panel2 p-3 text-[12.5px]">
        <span class="sof-pulse"><SofIcon name="alert" :size="14" /></span>
    ...
    <template #footer>
      <button class="sof-btn" @click="copyAll">
        <SofIcon name="copy" :size="12" /> 复制完整内容
      </button>
      <button v-if="phase === 'manual'" class="sof-btn primary" @click="copyCommand">
        <SofIcon name="copy" :size="12" /> 复制命令
      </button>
      <div class="flex-1" />
      <button class="sof-btn" @click="emit('close')">{{ phase === 'manual' ? '关闭' : '取消' }}</button>
      <button v-if="phase !== 'manual'" class="sof-btn primary" :disabled="phase === 'writing'" @click="emit('confirm')">
