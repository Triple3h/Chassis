  ...
  >
    <div class="flex flex-col gap-3">
      <div class="flex items-center gap-2">
        <textarea
          v-model="text"
          class="sof-input sof-mono h-40 resize-none leading-[18px]"
...
# 10.0.0.3	old.example.com"
      ...
      <div class="flex flex-wrap items-center gap-2 text-[12px]">
        <button class="sof-btn" @click="pullFromClipboard">
          <SofIcon name="clipboard" :size="12" /> 从剪贴板读取
        </button>
        <span class="sof-chip">识别 {{ parsed.entries.length }} 条</span>
        <span v-if="parsed.skipped.length" class="sof-chip text-warn">
      ...
      <div v-if="preview.length" class="sof-scroll max-h-40 rounded-lg border border-line bg-panel2 p-2">
        <div v-for="(entry, i) in preview" :key="i" class="sof-mono truncate py-0.5 text-[11.5px]">
      ...
      <div v-if="parsed.skipped.length" class="rounded-lg border border-line bg-panel2 p-2">
        ...
        <div v-for="(line, i) in parsed.skipped.slice(0, 6)" :key="i" class="sof-mono truncate text-[11.5px] text-muted">
          {{ line }}
        </div>
      </div>
    ...
    <template #footer>
      <button class="sof-btn" @click="copyInput">复制输入内容</button>
      <div class="flex-1" />
      <button class="sof-btn" @click="emit('close')">取消</button>
      <button class="sof-btn primary" :disabled="!parsed.entries.length" @click="submit">
