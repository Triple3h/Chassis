...
<template>
  <SofDialog title="快照" subtitle="每次写入前会自动存一份；载入后仍需点保存才会生效" @close="emit('close')">
    <div class="flex flex-col gap-3">
      <div class="flex items-center gap-2">
        ...
        />
        <button class="sof-btn primary shrink-0" @click="submit">
          <SofIcon name="save" :size="12" /> 存档
        </button>
      </div>
      ...
      <div v-else class="sof-scroll flex max-h-[46vh] flex-col gap-1.5">
        <div
        ...
        >
          <span :class="snap.kind === 'auto' ? 'text-faint' : 'text-accent'">
            <SofIcon :name="snap.kind === 'auto' ? 'history' : 'save'" :size="13" />
          </span>
          ...
          <button class="sof-btn" @click="emit('load', snap)">载入</button>
          <button class="sof-btn ghost text-danger" title="删除" @click="emit('remove', snap.id)">
    ...
    <template #footer>
      <div class="flex-1" />
      <button class="sof-btn" @click="emit('close')">关闭</button>
    </template>
  </SofDialog>
</template>
