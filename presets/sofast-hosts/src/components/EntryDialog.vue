...
<template>
  <SofDialog :title="line ? '编辑记录' : '新增记录'" subtitle="改完点保存，才会真正写进系统文件" @close="emit('close')">
    <div class="flex flex-col gap-3.5">
      <label class="flex flex-col gap-1.5">
        <span class="text-[12px] text-muted">IP 地址</span>
        <input
          v-model="ip"
          class="sof-input sof-mono"
      ...
      <label class="flex flex-col gap-1.5">
        ...
        <input v-model="comment" class="sof-input" placeholder="写给自己看的说明" @keydown.enter="submit" />
      </label>

      <div class="flex items-center gap-2">
        <button class="sof-switch" :data-on="enabled" @click="enabled = !enabled" />
        <span class="text-[12.5px]">{{ enabled ? '启用（写入后立即生效）' : '禁用（写入时保持注释状态）' }}</span>
      </div>
    </div>

    <template #footer>
      <button v-if="line" class="sof-btn danger" @click="emit('remove')">删除</button>
      <div class="flex-1" />
      <button class="sof-btn" @click="emit('close')">取消</button>
      <button class="sof-btn primary" :disabled="blocked" @click="submit">确定</button>
    </template>
  </SofDialog>
</template>
