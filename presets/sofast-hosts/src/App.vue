...
onMounted(async () => {
  ...
  stopSearchWatch = watchSearchContent((value) => {
    query.value = value
  })
})
...
async function syncSearchContent() {
  const initial = (await getSearchContent()).trim()
  if (!initial) return
  query.value = initial
...
async function isHosted(): Promise<boolean> {
  if (readSession().sid) return true
  return inHost()
}
...
async function reload(notify = false) {
  ...
  const res = await runScript<HostsReadResult>('hosts-read', {}, 15_000)
...
async function confirmWrite() {
  ...
  const res = await runScript<HostsWriteResult>('hosts-write', { content: nextText.value }, WRITE_TIMEOUT)
...
<template>
  <AppShell>
    ...
    <header class="flex h-12 shrink-0 items-center gap-2 border-b border-line px-3">
      <span class="text-accent"><SofIcon name="sliders" :size="15" /></span>
      <span class="shrink-0 text-[13px] font-semibold">Hosts 管家</span>

      <button class="sof-chip max-w-[190px] truncate" :title="path" @click="copyPath">
        <SofIcon name="file" :size="11" />
        {{ path }}
      </button>

      <span v-if="demoMode" class="sof-chip text-warn" title="没检测到如快宿主，改动不会写进系统文件">
        <SofIcon name="alert" :size="11" /> 演示模式
      </span>
      <span
        v-else-if="fileInfo && !fileInfo.writable"
        class="sof-chip text-warn"
        title="当前进程没有写权限，保存时会请求管理员授权"
      >
        <SofIcon name="lock" :size="11" /> 需授权
      </span>

      <div class="relative min-w-0 flex-1">
        <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-faint">
          <SofIcon name="search" :size="13" />
        </span>
        <input ref="searchRef" v-model="query" class="sof-input pl-7" :placeholder="placeholder" />
      </div>

      <button class="sof-btn" title="新增记录（⌘N）" @click="onCreate">
        <SofIcon name="plus" :size="12" /> 新建
      </button>
      <button class="sof-btn" title="批量粘贴" @click="showPaste = true">
        <SofIcon name="clipboard" :size="13" />
      </button>
      <button class="sof-btn" title="快照与回滚" @click="showSnapshots = true">
        <SofIcon name="history" :size="13" />
      </button>
      <button class="sof-btn" title="重新读取系统文件" @click="reload(true)">
        <SofIcon name="refresh" :size="13" />
      </button>
    </header>
    ...
    <div class="flex h-9 shrink-0 items-center gap-1.5 border-b border-line px-3">
      ...
      <button v-if="removed.length" class="sof-btn" @click="undoRemove">
        <SofIcon name="history" :size="12" /> 撤销删除 ({{ removed.length }})
      </button>
      <span v-if="changeCount" class="text-[11.5px] text-muted">{{ changeCount }} 处改动</span>
      <button class="sof-btn primary" :disabled="!isDirty" @click="openSave">
        <SofIcon name="save" :size="12" /> 保存
      </button>
    </div>

    <!-- 列表 -->
    <div ref="scroller" class="sof-scroll min-h-0 flex-1">
      ...
      <div v-else-if="!filtered.length" class="flex h-full flex-col items-center justify-center gap-2.5 text-muted">
        <SofIcon name="file" :size="22" class="text-faint" />
        <div class="text-[12.5px]">
          {{ query || filter !== 'all' ? '没有匹配的记录' : '这个 hosts 文件是空的' }}
        </div>
        <button v-if="!query && filter === 'all'" class="sof-btn" @click="onCreate">
          <SofIcon name="plus" :size="12" /> 添加第一条
        </button>
      </div>
    ...
    <div class="flex h-8 shrink-0 items-center gap-3 border-t border-line px-3 text-[11.5px] text-muted">
      ...
      <span v-if="conflicts.size" class="text-warn" title="同一个域名被分配了多个 IP，hosts 只会以最后一条为准">
        <SofIcon name="alert" :size="11" class="mr-1 inline" />
