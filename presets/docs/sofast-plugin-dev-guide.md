...
> 适用：如快 **v0.9.0+**（顶层 `commands` 清单 + 新 Worker API）
> 配套代码：本仓库 `presets/` 下三个真实插件
> 最后更新：2026-09-14
> 快速上手看 [`../.codebuddy/skills/sofast-plugin-dev/SKILL.md`](../.codebuddy/skills/sofast-plugin-dev/SKILL.md)，硬约束看 [`../.codebuddy/rules/sofast-plugin/RULE.mdc`](../.codebuddy/rules/sofast-plugin/RULE.mdc)
...
```bash
npm create sofast-extension     # 脚手架（框架选 Vue/React/Empty）
cd <plugin> && npm i
npm i @sofastapp/api            # UI 命令需要；No-View/Script 需要 @sofastapp/api/node
npm run dev                     # 本地开发（浏览器可直接访问，宿主 API 会降级）
npm run build                   # 产物在 dist/，这就是一个插件目录
```

安装：把 `dist/` 放进 `<如快安装目录>/extensions/<插件名>`，重启如快，命令面板搜 `commands[].title`。
...
```jsonc
{
  ...
  "type": "module",             // 必须，ESM
  "description": "...",
  "categories": ["tool"],       // 注意：旧模板里拼成了 categroies
  "commands": [
...
- **本地存储**：插件私有的键值存储，落盘在插件目录的 `data/storage.json`，**明文 JSON**，只支持可序列化值。
- **网络**：插件是本地静态页，可以联网，但把密钥类数据发出去等于自曝——本项目三个插件都刻意不联网。

---

## 2. API 速查（`@sofastapp/api@0.0.3`）

### 2.1 UI 侧（`import { Context, Screenshot, LocalStorage, Backend, Quicklink, API } from '@sofastapp/api'`）

```ts
// 搜索框
await Context.getSearchContent(): Promise<string>
await Context.setSearchContent(v: string): Promise<boolean>
await Context.clearSearchContent(): Promise<boolean>
Context.watchSearchContent(cb): () => void        // 返回取消函数

// 底部操作栏（仅 UI 环境）
await Context.setFooter(button | button[]): Promise<boolean>
//   { type:'button', label, icon, keys:['Mod+K'], onClick }
//   { type:'action-panel', label, keys, title, items:[{ name, icon, color, keys, onSelect }] }
//   icon 用 lucide 图标名；keys 同时是触发键与展示提示

// 截图（注意：只返回是否触发成功，拿不到图片数据）
await Screenshot.start(): Promise<boolean>

// 插件私有存储
await LocalStorage.getItem<T>(key): Promise<T | undefined>
await LocalStorage.setItem(key, value): Promise<void>
await LocalStorage.removeItem(key): Promise<void>
await LocalStorage.allItems<T>(): Promise<T>
await LocalStorage.clear(): Promise<void>

// 调用同插件的 script 命令
await Backend.run<T>(command, args?, { timeoutMs }): Promise<T>   // 返回值 = 脚本里 done(x) 的 x

// 快捷链接
Quicklink.all() / add(info) / edit(id, info) / remove(id)
```

顶层还导出：`getSessionId()`、`getCurrentCommand()`、`getRuntimeEnv()`、`inSofastIframe()`、`loadManifest()`、`postToHost()`、`subscribeHostMessage()`。
`API.call(method, params?, {timeoutMs})` / `API.fn(fn, {once})` / `API.expose(name, handler)` 是底层 RPC，**宿主方法名没有公开文档**，不要依赖。

### 2.2 Node 侧（`import { ctx, log, progress, done, fail, onError, LocalStorage } from '@sofastapp/api/node'`）

```ts
onError()                                   // 先注册，兜住未处理异常
const { command, args, pluginPath } = ctx() // args 来自 Backend.run 的第二个参数
...
宿主不在时（浏览器里 `npm run dev`、或旧版本宿主），SDK 的请求会 **postMessage 出去但永远没人应答**，Promise 永久 pending。表现就是：插件白屏、卡在加载态。

**所有宿主调用都必须包超时**，本仓库统一收口在 `shared/lib/platform.ts`：
- 超时后回落 `localStorage` / 空值；
- 用 `hostCall()` + 哨兵值区分「宿主没应答」和「正常返回 undefined」；
- `storage` 一旦确认过宿主可用（`inHostCache`），就不再写 localStorage，避免双份数据。

### 2.5 Script 命令能做的事（能力边界）

- ✅ 读写任意本地文件、遍历目录、调用 Node 生态、访问网络
- ✅ 通过 `ctx().pluginPath` 定位自己的数据目录
- ❌ 不能弹 UI、不能长期驻留（一次性执行，`done()` 后结束）
- ⚠️ 能力越强越要自己收紧：本项目 `read-image` 只做「扫描白名单目录 + 扩展名白名单 + 文件头魔数校验 + 单文件 20MB 上限」
- ⚠️ 需要改系统文件（如 hosts）时：**目标路径绝不能来自调用方入参**，只认平台默认路径 —— 否则「能提权写文件」的脚本就成了任意文件写入的跳板；提权一律走系统自带对话框（macOS `osascript … with administrator privileges`、Windows `-Verb RunAs`、Linux `pkexec`），并且**写前备份、写后回读逐字节校验**，提权不可用时回落到「把待生效内容落盘 + 给用户一条可复制的命令」。完整范例见 `presets/sofast-hosts/src/no-view/_hosts-file.ts`
...
- 安装：`dist/` → `<如快安装目录>/extensions/<插件名>`，**重启如快**（启动时扫描 `extensions/`）。
- 开发：`npm run dev` 起 Vite 服务器，浏览器直接访问即可调 UI；宿主相关能力会走降级分支。
...
├── scripts/      build-all.mjs / spec-check.mjs / manifest-plugin.mjs / vite-shared.mjs
└── docs/         本手册
```

预置插件是底座仓库的 pnpm workspace 成员，依赖由仓库根的 `pnpm install` 统一装。

### 4.2 `shared/` 的依赖解析（关键坑）

`shared/` 在插件的 `node_modules` 之外，Node 解析裸模块会一路向上找不到 `vue`。解决：

- **Vite**：`resolve.alias` 显式指向插件自己的 `node_modules`（`scripts/vite-shared.mjs` 的 `sharedAliases()`）
- **TS**：`tsconfig.paths` 同步映射 `vue` / `@sofastapp/api` / `@launcher/api`
- **dev server**：`server.fs.allow` 放开到 `presets/`，否则读取 `shared/` 会被拦
...
| 文件 | 作用 |
|---|---|
| `shared/lib/platform.ts` | 宿主能力封装（超时 + 降级 + 存储回落） |
...
| `shared/ui/SofIcon.vue` | 自绘图标集（零依赖，避免为 20 个图标引入整个图标库） |
| `shared/ui/SofDialog.vue` | 弹窗骨架（Esc 关闭、尺寸档位） |
...
- **原因**：`LocalStorage.getItem()` 等宿主调用在无宿主环境下 Promise 永不 resolve。
- **对策**：所有宿主调用加超时（`withTimeout` + 哨兵值），失败回落本地实现。见 `shared/lib/platform.ts`。
...
- **对策**：用 `mode: "script"` 的 Node Worker 读盘，转 base64 回传（本仓库 `presets/sofast-totp/src/no-view/read-image.ts`）。
...
- **技巧**：命令声明 `searchable: true`，插件启动时 `Context.getSearchContent()` 读一次，用完 `clearSearchContent()`；再用 `watchSearchContent()` 监听后续输入。这样「粘一条链接/密钥 → 回车 → 插件直接处理」的体验才成立。

### 5.16 清单字段拼写

- 官方旧模板里写的是 `categroies`（拼错），新示例用 `categories`。不影响命令注册，但别被模板带歪。
...
- **对策**：**逐入口各跑一次构建**，每个产物自包含。本仓库做法（`presets/sofast-hosts`）：
  - `vite.worker.config.ts` 用 `build.lib.entry` 指向**单个**入口，输出名由 `fileName` 固定，入口由环境变量 `SOFAST_WORKER_ENTRY` 选择；
...
```
[ ] presets/<name>/package.json：commands 齐全，mode 正确
...
[ ] tsconfig.json：extends shared/tsconfig.base.json + paths（vue / @sofastapp/api / @shared）
[ ] src/styles/app.css：@import tailwindcss + theme.css，@source 覆盖 src 与 shared
[ ] 宿主调用全部经 shared/lib/platform.ts（不许裸调 SDK）
[ ] 大计算进 Worker、长列表虚拟滚动
[ ] npm run typecheck && npm run build：dist 里 index.html + assets + package.json(+ .mjs)
...
- 官方文档：https://developer.sofast.fun
  - [快速开始](https://developer.sofast.fun/guide/quickstart) ｜ [清单与目录结构](https://developer.sofast.fun/guide/manifest) ｜ [开发 View 命令](https://developer.sofast.fun/guide/view) ｜ [开发 No-View 命令](https://developer.sofast.fun/guide/no-view) ｜ [在 View 中调用 Script](https://developer.sofast.fun/guide/script) ｜ [打包与安装](https://developer.sofast.fun/guide/deploy) ｜ [实践教程](https://developer.sofast.fun/guide/hands-on)
  - API：[总览](https://developer.sofast.fun/api/) ｜ [上下文](https://developer.sofast.fun/api/context) ｜ [截图](https://developer.sofast.fun/api/screenshot) ｜ [本地存储](https://developer.sofast.fun/api/storage)
- 官方示例：https://github.com/Blushyes/sofast-extensions
- 本仓库四个真实插件：`presets/sofast-totp`（含 script 命令）、`presets/sofast-text-diff`（Worker + 虚拟滚动）、`presets/sofast-json-tools`（无损解析 + 树视图）、`presets/sofast-hosts`（读盘 + 提权写入 + 无损回写）
