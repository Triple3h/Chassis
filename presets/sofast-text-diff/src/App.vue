...
import { copyText, downloadBlob, fileFromDataTransfer, pickFile, readClipboardText, textFromDataTransfer } from '@shared/lib/clipboard'
import { clearSearchContent, getSearchContent, setFooter, watchSearchContent } from '@shared/lib/platform'
import { isMod, modLabel } from '@shared/lib/keys'
...
async function syncFooter() {
  await setFooter([
    ...
    },
    {
      ...
      title: '文本比对',
      items: [
        { id: 'swap', name: '交换两侧', keys: ['Mod+Shift+S'], onSelect: swap },
...
onMounted(async () => {
  ...
  const seed = (await getSearchContent()).trim()
...
<template>
  <AppShell>
    <header class="flex flex-wrap items-center gap-2 border-b border-line px-3 py-2">
      <SofIcon name="branch" :size="16" class="text-accent" />
      <span class="text-[13px] font-semibold">文本比对</span>
      <span v-if="busy" class="sof-chip">计算中…</span>
      <span v-else-if="result" class="sof-chip">{{ result.ms }}ms</span>
      ...
      <div class="ml-auto flex items-center gap-1.5">
        <button class="sof-btn ghost" :class="{ primary: ignoreCase }" title="忽略大小写" @click="ignoreCase = !ignoreCase">
          Aa
        </button>
        <button
          class="sof-btn ghost"
        ...
        >
          <SofIcon name="wrap" :size="13" />
        </button>
        <button class="sof-btn ghost" :class="{ primary: collapse }" title="折叠相同行" @click="collapse = !collapse">
          <SofIcon name="minus" :size="13" />
        </button>
        <button class="sof-btn ghost" title="交换两侧" @click="swap"><SofIcon name="refresh" :size="13" /></button>
        <button class="sof-btn ghost" title="复制 unified diff" @click="copyUnified">
          <SofIcon name="copy" :size="13" />
        </button>
        <button class="sof-btn ghost" title="导出 .patch" @click="downloadPatch">
          <SofIcon name="download" :size="13" />
        </button>
        <button class="sof-btn ghost" title="载入示例" @click="loadSample"><SofIcon name="wand" :size="13" /></button>
        <button class="sof-btn ghost" title="清空" @click="clearAll"><SofIcon name="trash" :size="13" /></button>
    ...
    <div v-if="seedNotice" class="flex items-center gap-2 border-b border-line px-3 py-1.5 text-[11.5px] text-muted">
      <SofIcon name="info" :size="13" />{{ seedNotice }}
    </div>
      ...
      >
        <div class="flex items-center gap-2 border-b border-line px-3 py-1.5 text-[11.5px] text-muted">
          ...
          <span class="ml-auto flex items-center gap-1">
            <button class="sof-btn ghost !px-1.5" title="粘贴剪贴板" @click="pasteInto(side)">
              <SofIcon name="clipboard" :size="12" />
            </button>
            <button class="sof-btn ghost !px-1.5" title="打开文件" @click="openInto(side)">
              <SofIcon name="folder" :size="12" />
            </button>
            <button
              class="sof-btn ghost !px-1.5"
              title="清空"
              @click="side === 'a' ? (textA = '') : (textB = '')"
            >
              <SofIcon name="close" :size="12" />
            </button>
          </span>
        </div>
        <textarea
    ...
    <div v-else class="min-h-0 flex-1">
      ...
      <div v-else class="grid h-full place-items-center px-6 text-center">
        <div class="space-y-2">
          <SofIcon name="branch" :size="26" class="mx-auto text-faint" />
          ...
          <button class="sof-btn mx-auto" @click="view = 'edit'">
            <SofIcon name="pencil" :size="13" />去填写文本
          </button>
        </div>
      </div>
    </div>
    ...
    <footer class="flex items-center gap-3 border-t border-line px-3 py-1.5 text-[11.5px] text-faint">
      ...
      <span class="ml-auto flex items-center gap-3">
        <span><span class="sof-kbd">{{ modLabel }}</span> <span class="sof-kbd">↵</span> 比对</span>
