...
function leftHtml(item: DisplayRow): string {
  ...
  return segsToHtml(row.leftSegs, row.left, 'sof-seg-del')
}

function rightHtml(item: DisplayRow): string {
  const row = item.source
  if (!row || row.kind === 'skip') return ''
  return segsToHtml(row.rightSegs, row.right, 'sof-seg-add')
}
...
function leftBg(item: VisibleItem): string {
  ...
  if (kind === 'del') return 'sof-row-del'
  if (kind === 'mod') return 'sof-mod-del'
  if (kind === 'skip') return 'sof-row-skip'
  return ''
}
...
function rightBg(item: VisibleItem): string {
  const kind = item.row.kind
  if (kind === 'ins') return 'sof-row-add'
  if (kind === 'mod') return 'sof-mod-add'
  if (kind === 'skip') return 'sof-row-skip'
  return ''
}
...
function unifiedBg(item: VisibleItem): string {
  ...
  if (item.row.kind === 'del') return isMod ? 'sof-mod-del' : 'sof-row-del'
  if (item.row.kind === 'ins') return isMod ? 'sof-mod-add' : 'sof-row-add'
  if (item.row.kind === 'skip') return 'sof-row-skip'
  return ''
}
...
<template>
  <div ref="scroller" class="sof-scroll relative h-full w-full bg-panel">
    <div class="relative" :style="{ height: totalHeight + 'px', width: widthCh + 'ch', minWidth: '100%' }">
      ...
      <template v-if="mode === 'split'">
        <div
          ...
          :key="item.row.key"
          class="sof-diff-row absolute inset-x-0 flex"
          :style="{ top: item.index * ROW + 'px' }"
        >
          <span class="sof-diff-no sticky left-0 bg-panel">{{ item.row.leftNo || '' }}</span>
          <span class="sof-diff-line w-1/2" :class="leftBg(item)" v-html="leftHtml(item.row)" />
          <span class="sof-diff-no sticky left-0 border-l border-line bg-panel">{{ item.row.rightNo || '' }}</span>
          <span class="sof-diff-line flex-1" :class="rightBg(item)" v-html="rightHtml(item.row)" />
        </div>
      </template>
      ...
      <template v-else>
        <div
          ...
          :style="{ top: item.index * ROW + 'px' }"
        >
          <span class="sof-diff-no sticky left-0 bg-panel">{{ item.row.leftNo || item.row.rightNo || '' }}</span>
          <span class="sof-diff-line flex-1" :class="unifiedBg(item)">
