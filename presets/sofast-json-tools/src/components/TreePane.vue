...
function kindClass(kind: number) {
  switch (kind) {
    case KIND.string:
      return 'text-[color:var(--sof-string)]'
    case KIND.number:
      return 'text-[color:var(--sof-number)]'
    case KIND.boolean:
    case KIND.null:
      return 'text-[color:var(--sof-code)]'
    default:
      return 'text-muted'
  }
}
  ...
  <div class="flex h-full w-full flex-col bg-panel">
    <div class="flex items-center gap-2 border-b border-line px-3 py-2">
      <div class="relative flex-1">
        ...
        <input v-model="filter" class="sof-input pl-7" placeholder="过滤键或值…" spellcheck="false" />
      </div>
      <button class="sof-btn ghost" title="展开两层" @click="expandDepth(2)">
        <SofIcon name="chevronDown" :size="13" />2 层
      </button>
      <button class="sof-btn ghost" title="全部展开" @click="expandAll">展开</button>
      <button class="sof-btn ghost" title="全部折叠" @click="collapseAll">折叠</button>
      <span class="sof-chip">{{ tree?.nodeCount ?? 0 }} 节点</span>
    </div>

    <div ref="scroller" class="sof-scroll relative flex-1 outline-none" tabindex="0">
      <div class="relative" :style="{ height: totalHeight + 'px' }">
        ...
        >
          <span :style="indentStyle(tree?.depth[idx] ?? 0)" class="flex items-center gap-1.5 min-w-0">
            ...
            <span class="text-[color:var(--sof-key)]">{{ tree?.key[idx] }}</span>
