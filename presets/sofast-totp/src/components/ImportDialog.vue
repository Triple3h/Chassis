...
import { inHost, runScript, triggerScreenshot } from '@shared/lib/platform'
...
/** script 命令只在如快宿主里跑得起来；浏览器调试时提前说清楚，别让用户白等超时 */
const SCRIPT_HINT = '当前不在如快宿主里（浏览器调试），读取本地路径需要如快 v0.9+；请改用「选择图片」或 ⌘V 粘贴'
...
async function listLocalImages() {
  ...
  const res = await runScript<ReadImageResult>('read-image', { listOnly: true, withinMinutes: 120, limit: 8 }, 12_000)
...
async function readLocalPath(filePath: string) {
  ...
  const res = await runScript<ReadImageResult>('read-image', { path: target }, 20_000)
  localBusy.value = false
  if (!res) {
    localNote.value = '读取失败：本地读取需要如快宿主支持 script 命令'
    return
  }
  ...
  >
    <div class="space-y-3" @drop="onDrop" @dragover.prevent>
      <div class="flex flex-wrap items-center gap-2">
        <button class="sof-btn primary" @click="hostScreenshot">
          <SofIcon name="camera" :size="13" />宿主截图
        </button>
        <button class="sof-btn" @click="fromClipboard">
          <SofIcon name="clipboard" :size="13" />粘贴剪贴板
        </button>
        <button class="sof-btn" @click="fromFile">
          <SofIcon name="folder" :size="13" />选择图片
        </button>
        <button class="sof-btn" :class="{ primary: showLocal }" @click="listLocalImages">
          <SofIcon name="file" :size="13" />本地图片
        </button>
        <button class="sof-btn" :class="{ primary: cameraOn }" @click="toggleCamera">
          <SofIcon name="qr" :size="13" />{{ cameraOn ? '停止摄像头' : '摄像头' }}
        </button>
      ...
      <div v-if="showLocal" class="rounded-lg border border-line">
        <div class="flex items-center gap-2 border-b border-line px-3 py-2 text-[12px]">
          <SofIcon name="file" :size="13" class="text-muted" />
          <span class="font-medium">本地已保存的截图</span>
          <button class="sof-btn ghost ml-auto !text-[11.5px]" :disabled="localBusy" @click="listLocalImages">
            <SofIcon name="refresh" :size="12" />重新扫描
          </button>
        </div>
        ...
        <div v-if="localFiles.length" class="sof-scroll max-h-40">
          <button
          ...
          >
            <SofIcon name="file" :size="12" class="text-faint" />
            <span class="min-w-0 flex-1 truncate">{{ f.name }}</span>
            <span class="text-faint">{{ fmtTime(f.mtime) }}</span>
            <span class="sof-chip">{{ fmtSize(f.size) }}</span>
          </button>
        </div>
        ...
        <div class="flex items-center gap-2 border-t border-line px-3 py-2">
          <input
            v-model="localPath"
            class="sof-input flex-1 font-mono !text-[11.5px]"
            placeholder="或直接填绝对路径，如 /Users/你/Desktop/截图.png"
            spellcheck="false"
            @keydown.enter="readLocalPath(localPath)"
          />
          <button class="sof-btn" :disabled="localBusy || !localPath.trim()" @click="readLocalPath(localPath)">
            <SofIcon name="download" :size="13" />读取
          </button>
        </div>
      </div>
      ...
      <video
        v-if="cameraOn"
        ref="videoEl"
        class="sof-scan-box h-44 w-full object-cover"
        playsinline
        muted
      />
      <div v-else-if="previewUrl" class="sof-scan-box flex max-h-44 items-center justify-center overflow-hidden">
        <img :src="previewUrl" alt="待识别图片" class="max-h-44 object-contain" />
      </div>
      <div v-else class="sof-scan-box sof-pulse grid h-24 place-items-center text-[12px] text-faint">
        截图 / 图片会显示在这里
      </div>
      ...
      <div v-if="drafts.length" class="rounded-lg border border-line">
        <div class="flex items-center gap-2 border-b border-line px-3 py-2 text-[12px]">
          ...
          <span v-if="batches > 1" class="sof-chip">迁移二维码共 {{ batches }} 张</span>
          <button class="sof-btn ghost ml-auto !text-[11.5px]" @click="toggleAll">
            {{ checked.size === drafts.length ? '全不选' : '全选' }}
          </button>
        </div>
        <div class="sof-scroll max-h-52">
          <label
            v-for="(d, i) in drafts"
            :key="i"
            class="sof-row flex cursor-pointer items-center gap-2 border-b border-line px-3 py-2 last:border-0 hover:bg-hover"
          >
            <input
              type="checkbox"
            ...
            <span class="sof-chip">{{ d.type === 'hotp' ? 'HOTP' : 'TOTP' }}</span>
            <span class="sof-chip">{{ d.digits !== 8 ? 6 : 8 }} 位</span>
            <span class="sof-code text-[11.5px] text-faint">{{ mask(d.secret) }}</span>
            <span v-if="isDuplicate(d)" class="sof-chip !text-warn">已存在</span>
          </label>
        </div>
      </div>
      ...
      <details class="rounded-lg border border-line px-3 py-2">
        ...
        <textarea
          v-model="manual"
          class="sof-input sof-scroll mt-2 h-20 resize-none font-mono"
          spellcheck="false"
          placeholder="otpauth://totp/... 或 otpauth-migration://offline?data=... 或 JBSWY3DPEHPK3PXP（可多行）"
        />
        <button class="sof-btn mt-2" @click="ingestManual">解析</button>
    ...
    <template #footer>
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        <button class="sof-btn primary" :disabled="!checkedCount" @click="confirmImport">
          <SofIcon name="check" :size="13" />导入 {{ checkedCount }} 个
        </button>
      </div>
    </template>
  </SofDialog>
