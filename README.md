...
- **开箱可用** —— 15 个出厂插件：应用启动、文件搜索、网址直达、聚合翻译、TOTP 验证码、Hosts 块管家、文本比对、JSON 工具箱、备忘快贴、计算稿纸、Markdown 笔记、ToDo 待办、录屏助手（macOS 专属）、剪贴板历史（Windows 专属）、设置与插件管理。
...
搜「更新」（或设置页 → 插件）打开更新页（页面顶部即「内核」区）：拉索引 → 下载 → 校验 → **不重启 App 生效**。
...
同一个更新页里还有「**内核**」一项：独立通道（固定 tag `kernel-latest`），拉 `kernel-registry.json` → 下载内核包
（`launcher-kernel` + `ui/` 同包）→ 校验 sha256 → **优雅重启内核**（不重启 App）→ 就绪即确认。

- **为什么内核也能单独更新**：内核热更新（`/api/hot/*`）支持不重启进程地替换路由 / 中间件 / 事件订阅；
  二进制更新则替换内核并优雅重启（壳自动把新内核拉起来、窗口导航到新端口）。
- **打包版怎么换**：内核会从 `.app` 复制一份到数据目录作为**外置副本**再启动，热更新只动数据目录 ——
  不触碰 App 的代码签名与系统授权。
...
plugins/            15 个出厂插件（见下表；view 是 Vite + Vue，逻辑层是 Rust 可执行产物）
tests/
...
| `clipboard-history`（Windows 专属，清单声明 `platforms`） | `history`（view）+ `search`（script，贡献型）+ `record` / `clip-io`（script） | Vite + Vue / Rust |
