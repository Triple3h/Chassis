...
| **App (shell), update** | `gh workflow run app-release.yml --ref main` (or tag `app/*`) | `app-registry.json` + `Chassis-<ver>-macos-<arch>.zip` → `app-latest` | 更新页 →「应用」, or from the **tray menu / About tab** — the kernel watch (90s after boot, every 6h) only **checks and prompts**; the shell downloads and swaps the `.app` via an out-of-process helper only after the user confirms (macOS only; `config.autoUpdateCheck`) |
