 ...
 *
 * **这份表是唯一源**：宿主结果网格（`apps/launcher-ui/src/components/IconGlyph.vue`）与
 * 设置页插件列表（`plugins/internal-settings`）都从这里取 —— 同一个插件在两处必须是同一个图标。
 * （Vue 侧的 `ui/UiIcon.vue` 是另一回事：那是插件给自己页面用的小图标集，与插件清单的 `icon` 无关。）
 */
export const ICONS: Record<string, string> = {
  ...
  // 「恢复默认大小」：四角外扩的框（lucide maximize），语义是"回到默认的窗口形态"
...
/** 查表并转成可直接 v-html / innerHTML 的 svg 子元素；未命中返回 null（调用方画首字母占位） */
...
/**
 * 完整 `<svg>` 外壳（原生 DOM 页面用；Vue 侧用 `iconMarkup` 配 `v-html` 自行渲染）。
 * 描边参数与 `IconGlyph.vue` / `UiIcon.vue` 保持一致：24 viewBox、currentColor、1.7 圆头。
 */
export function iconSvg(name: string | undefined, size: number, weight = 1.7): string | null {
  const markup = iconMarkup(name)
  if (!markup) return null
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${weight}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${markup}</svg>`
}
