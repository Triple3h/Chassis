...
| 托盘 | 左键点击 = 唤出/隐藏；右键菜单 = 设置 / 插件管理 / 重载全部插件 / 退出。菜单由内核提供、壳只渲染转发；「设置」「插件管理」= 内核唤出窗口后广播 `ui/openView`（载荷 = invoke 的 `ActionResult`），UI 用它打开对应插件页。**进行中会话区**（plugin-spec §3.6）：插件声明 `session` 后，托盘**最前面**多一行「插件名 · 状态词 实时时长」+ 暂停 / 继续 / 结束（如录屏中），会话结束（`active: false`）整块撤掉；1s 一拍本地走字，每 2 拍才拉起一次插件状态命令 |
| 单实例 | 第二次启动只唤起已运行实例 |
| 开机自启 | 可配置，默认关 |
...
**日志（关于页）**：提供「导出最近一次会话 / 导出全部日志」两个范围（前者 = 本次内核运行期的内存缓冲，
后者 = `<dataRoot>/logs/kernel.log` 跨运行全量 + `audit-*.jsonl`）。内核把**插件加载与状态、内核运行日志、
错误与审计摘要**汇总成一个文本文件（`<dataRoot>/logs/exports/chassis-logs-*.txt`，只保留最近 10 份）并在
访达 / 资源管理器中显示 —— 用户可直接把该文件发给开发者或 AI 助手排查插件 / 内核问题。
内核日志落盘在 `<dataRoot>/logs/kernel.log`（超过 2MB 启动时轮转成 `kernel.log.1`；壳日志仍是同目录 `shell.log`）。
