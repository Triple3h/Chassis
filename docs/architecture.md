...
| D20 | §6.1「壳只提供系统原语，零业务逻辑」 | **应用（壳）自更新归壳**（2026-09-19）：检查 / 下载 / 解压由内核侧编排（复用 `internal-store` 的 `check-app` / `download-app`，同一套代理降级 + 白名单 + sha256），**候选包自检 / 台账 / 替换 / 重启在壳**（`apps/shell/src/update.rs` + 脱离壳进程树的 helper） | 壳就是应用本体，「换自己」只能是它的职责：它要在内核起不来时也能更新自己，而替换动作必须发生在壳**完全退出之后**（内核随后被停掉，整个应用一起重启）。这不是业务逻辑而是自身运维，与「零能力」不冲突；触发时机由内核守护掌握（`spawn_app_update_watch`，`config.autoUpdateApp` 可关），机制细节见 `docs/shell-hot-update.md` |
